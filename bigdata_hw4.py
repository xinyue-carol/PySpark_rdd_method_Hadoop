...
if __name__=='__main__':
    ...
    rest_data=data0.filter(lambda x: x[0] in rest_list)\
                    ...
                    .groupByKey()\
                    .mapValues(list) \
                    .map(lambda x: (x[0][:4], datetime.datetime.strptime(x[0], "%Y-%m-%d").date(), int((median(x[1]) + std(x[1]))),
                        int((median(x[1]) - std(x[1]))), int(median(x[1])))) \
    ...
    df_rest1 = spark.createDataFrame(rest_data_sort, ['year', 'date','high','low','median'])
    df_rest = df_rest1.sort('year', 'date')

    df_bigbox1 = spark.createDataFrame(bigbox_data_sort, ['year', 'date','high','low','median'])
    df_bigbox = df_bigbox1.sort('year', 'date')

    df_cstore1 = spark.createDataFrame(cstore_data_sort, ['year', 'date','high','low','median'])
    df_cstore = df_cstore1.sort('year', 'date')

    df_drink1 = spark.createDataFrame(drink_data_sort, ['year', 'date','high','low','median'])
    df_drink= df_drink1.sort('year', 'date')

    df_lrest1 = spark.createDataFrame(lrest_data_sort, ['year', 'date','high','low','median'])
    df_lrest= df_lrest1.sort('year', 'date')

    df_pnd1 = spark.createDataFrame(pnd_data_sort, ['year', 'date','high','low','median'])
    df_pnd= df_pnd1.sort('year', 'date')

    df_snb1 = spark.createDataFrame(snb_data_sort, ['year', 'date','high','low','median'])
    df_snb= df_snb1.sort('year', 'date')

    df_sfs1 = spark.createDataFrame(sfs_data_sort, ['year', 'date','high','low','median'])
    df_sfs= df_sfs1.sort('year', 'date')

    df_superm1 = spark.createDataFrame(superm_data_sort, ['year', 'date','high','low','median'])
